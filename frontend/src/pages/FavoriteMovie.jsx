...
const FavoriteMovie = () => {
    ...
    const GetData = () => {
        axios.get(`${ServerAddress}/favoriteMovie/get-data/${accountId}`)
            .then((result) => {
                console.log(result);
                setFavoriteMovies(result.data.data);
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    const removeFavoriteMovie = (account, movie) => {
        const data = {
            account: account,
            movie: movie
        }
        axios.post(`${ServerAddress}/favoriteMovie/delete-movie`, data)
            .then((result) => {
                console.log(result);
                alert("Xoá phim khỏi danh sách yêu thích thành công.");
                GetData();
            })
            .catch((error) => {
                alert('error');
            ...
            <main>
                <div>
                    <div className='w-[1200px] p-3 mt-3 mb-3 h-full mx-auto bg-[#414141]'>
                        <span className='ml-5 text-[#ffc23d] font-bold text-3xl'>Phim yêu thích</span>
                        <div className='flex'>
                            <div className='mt-5 w-[69%] h-full grid grid-cols-5 overflow-hidden p-5 rounded-lg'>
                                {favoriteMovies.map((movie, index) => (
                                    <div key={index} className='relative inline-block ml-3'>
                                        <a href={`/phim/${movie.movie.path}`} className='block'>
                                            <img className="w-[140px] h-[180px] object-cover" src={movie.movie.poster} alt="Movie" />
                                            <span className='text-white'>
                                                {movie.movie.title.length >= 16 ? movie.movie.title.slice(0, 16) + "..." : movie.movie.title}
                                            </span>
                                        </a>
                                        <span
                                            onClick={() => removeFavoriteMovie(accountId,movie.movie._id)}
                                            className='absolute top-0 right-0 w-8 h-8 mr-1 mt-1 bg-red-600 rounded-full flex items-center justify-center text-white font-bold cursor-pointer'
                                        >
                                            X
                                        </span>
                                    </div>
