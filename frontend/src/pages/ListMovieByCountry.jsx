...
const ListMovieByCountry = () => {
  ...
  useEffect(() => {
    axios.get(`${ServerAddress}/country/get-by-path/${country}`)
    .then((result) => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${ServerAddress}/movie/quoc-gia/${country}`)
      .then((response) => {
        setLoading(true);
          ...
          <div className='w-[1200px] p-3 mt-3 mb-3 h-full mx-auto bg-[#414141]'>
            <div className='flex'>
              <div className=' w-[69%] h-full overflow-hidden'>
                {/*Phim lẻ */}
                <div>
                  <span className='text-3xl ml-4 text-white font-bold'>Phim {countryName}</span>
